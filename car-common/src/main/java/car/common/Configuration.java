...
public class Configuration {
    ...
    public Configuration(String args[]) throws Exception {
        ...
        if (!help) {
            for (Field f : allFields) {
                Argument argConf = f.getAnnotation(Argument.class);
                if (!(f.getAnnotation(Argument.class).required() && get(f, this) == null)) continue;
    ...
    private Object get(Field f, Object o) throws IllegalAccessException {
        boolean accessible = f.isAccessible();
        f.setAccessible(true);
        Object ret = f.get(o);
        f.setAccessible(accessible);
    ...
    public Map<String, String> getAsMap() throws Exception {
        ...
        for (Field f : getFields()) {
            ...
            Object v = get(f, this);
            if (v != null)
                ret.put(key.substring(key.lastIndexOf("-") + 1), v.toString());
        }
        ret.putAll(curValues);
        return ret;
    }
}