...
import car.app.service.api.UserServiceUtil;
...
public class UserController {
    ...
    public List<AutoCompleteResult> getUserGroupSuggestions(@RequestParam("term") String sQuery) {
        ...
        US.findUsersLike(stripQ(sQuery)).stream().map(u -> new AutoCompleteResult(u.getFullName(), UserServiceUtil.getUserTag(u.getUserName()))).forEach(ret::add);
        US.findGroupsLike(stripQ(sQuery)).stream().map(g -> new AutoCompleteResult(g, UserServiceUtil.getGroupTag(g))).forEach(ret::add);
        return ret;
    }
