...
public class TypesController {
    ...
    Reflections ref = new Reflections("car", new SubTypesScanner());
    ...
    private void eatSchema(Class<?> C) {
        ...
        clazz.setSubTypes(getTypeNames(C));

        eatFields(C, fields);
    }

    private void eatFields(Class<?> C, Map<String, PField> fields) {
        if (C == null || !C.getCanonicalName().startsWith("car"))
            return;
    ...
    private TypeInfo getTypeInfo(Field f) {
        ...
        if (Collection.class.isAssignableFrom(typ)) {
            return buildTypeNamesForGenericObject("list", ((ParameterizedType) f.getGenericType()).getActualTypeArguments());
        } else if (Map.class.isAssignableFrom(typ)) {
            return buildTypeNamesForGenericObject("map", ((ParameterizedType) f.getGenericType()).getActualTypeArguments());

        }
        return TypeInfo.builder().singleValued(true).typeNames(getTypeNames(typ)).build();
    }

    private TypeInfo buildTypeNamesForGenericObject(String name, Type[] types) {
    ...
    public TypesController() {
        eatSchema(Rule.class);
        eatSchema(RecordReader.class);
        eatSchema(Detector.class);
        eatSchema(FieldType.class);
        eatSchema(Visualization.class);
        System.out.println(new GsonBuilder().setPrettyPrinting().create().toJson(types));
    }

    @RequestMapping("/types")
    public Map<String, PClass> uiConfig() {
        return types;
    }
